      ...
      const renderCandidates = async () => {
        try {
          ...
          for (const candidate of candidates) {
            ...
            // Master OFF locks the whole bank: individual switches are inert.
            toggleBtn.disabled = !executionAllowed;
            ...
            for (const model of group.models ?? []) {
              ...
              try {
                await fetch('/plugins/arena/candidates', {
                  method: 'POST',
                  headers: { 'content-type': 'application/json' },
                  body: JSON.stringify({ provider: group.id, model: model.id, enabled: executionAllowed }),
                });
