...
import { paretoFront, recommendWinner } from './pareto.ts';
import type { ArenaRun, Experiment, MetricDirections } from './types.ts';

/** Represents one complete export without writing to disk on the caller's behalf. */
export interface ArenaReport { experiment: Experiment; runs: ArenaRun[]; paretoRunIds: string[]; winnerRunId?: string; auditAlerts: string[]; }

/** Builds a redacted report and identifies the deterministic Pareto frontier. */
export function createReport(experiment: Experiment, runs: ArenaRun[], directions: MetricDirections = {}): ArenaReport {
  const redactedRuns = redactValue(runs) as ArenaRun[];
  const frontier = paretoFront(redactedRuns, directions);
  return { experiment: redactValue(experiment) as Experiment, runs: redactedRuns, paretoRunIds: frontier.map((run) => run.runId), winnerRunId: recommendWinner(frontier)?.runId, auditAlerts: redactedRuns.flatMap((run) => run.auditAlerts ?? []) };
}

/** Serializes the full redacted report as readable JSON. */
export function exportJson(report: ArenaReport): string { return `${JSON.stringify(report, null, 2)}\n`; }

/** Serializes one redacted run per line for append-friendly ingestion. */
export function exportJsonl(report: ArenaReport): string {
...
export function exportMarkdown(report: ArenaReport): string {
...
function metric(value: number | undefined): string { return value === undefined ? '—' : String(value); }
