...
import { createRunId } from './experiment.ts';
import { evaluateGates, summarizeGateStatus } from './gates.ts';
import type { ArenaStore } from './arena.ts';
import type { ArenaRun, CandidateProfile, Experiment, GateEvidence, GatePolicy, RunMetrics } from './types.ts';

/** Input delivered only to a caller-supplied DSH adapter. */
export interface RunExecutionRequest { experiment: Experiment; candidate: CandidateProfile; runId: string; signal: AbortSignal; }

/** Auditable output expected from an explicit execution adapter. */
export interface RunExecutionResult { evidence: GateEvidence; metrics?: RunMetrics; output?: string; auditAlerts?: string[]; }

/** A confirmed adapter owns all DSH invocation details; Arena never guesses a command. */
export type ArenaRunExecutor = (request: RunExecutionRequest) => Promise<RunExecutionResult>;

/** Queue controls with conservative serial execution by default. */
export interface OrchestratorOptions { maxConcurrency?: number; timeoutMs?: number; gatePolicy?: GatePolicy; }

interface QueueItem { runId: string; candidate: CandidateProfile; experiment: Experiment; ordinal: number; controller?: AbortController; cancelled?: boolean; }
...
export class ArenaOrchestrator {
  ...
  private readonly executor: ArenaRunExecutor;
  ...
  constructor(store: ArenaStore, executor: ArenaRunExecutor, options: OrchestratorOptions = {}) {
    this.store = store;
  ...
  enqueue(candidates?: CandidateProfile[]): string[] {
    ...
    const ids = selected.map((candidate, index) => {
      const item: QueueItem = { candidate: structuredClone(candidate), experiment: structuredClone(snapshot.experiment!), ordinal: offset + index, runId: createRunId(snapshot.experiment!.experimentId, candidate, offset + index) };
  ...
  private async execute(item: QueueItem): Promise<void> {
    ...
    try {
      const timeoutPromise = this.options.timeoutMs === undefined ? undefined : new Promise<never>((_, reject) => {
        timeout = setTimeout(() => {
  ...
  private finishCancelled(item: QueueItem, startedAt?: string, started?: number): void {
    const experiment = item.experiment;
  ...
  private makeRun(experiment: Experiment, item: QueueItem, state: 'queued' | 'running'): ArenaRun {
  ...
  private recordIfCurrent(run: ArenaRun): boolean {
  ...
  private isCurrent(experimentId: string): boolean {
