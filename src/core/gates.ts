...
import type { GateEvidence, GatePolicy, GateResult, VerificationStatus } from './types.ts';

/** Evaluates all configured gates without executing arbitrary commands. */
export function evaluateGates(policy: GatePolicy, evidence: GateEvidence): GateResult[] {
...
export function summarizeGateStatus(results: GateResult[]): VerificationStatus {
...
function result(name: string, status: VerificationStatus, message: string): GateResult {
