...
const DEFAULT_DIRECTIONS                             = { quality: 'max', success: 'max', durationMs: 'min', tokens: 'min', cost: 'min', errors: 'min' };

/** Returns true only when left is at least as good on every observed metric and better on one. */
export function dominates(left          , right          , directions                   = {})          {
  let strictlyBetter = false;
  for (const metric of Object.keys(DEFAULT_DIRECTIONS)                ) {
...
export function paretoFront(runs            , directions                   = {})             {
...
export function recommendWinner(frontier            )                       {
...
//# sourceURL=E:\agent\DSH_UI\dsh-arena\src\core\pareto.ts