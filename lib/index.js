...
export function apply(ctx         , config         = {})       {
  ...
  // DSH_HOME-aware: honor the env override (moved homes) and fall back to the
  // conventional ~/.dsh so every host shares one policy file.
  const arenaHome = process.env.DSH_HOME?.trim() || join(homedir(), '.dsh');
  const policyFile = join(arenaHome, 'arena-policy.json');
  let policyMtime = 0;
