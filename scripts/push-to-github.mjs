...
const { stdout: filesOut } = await ex("git", ["-c", "core.quotepath=false", "ls-files"], { cwd: process.cwd() });
...
if (!parentSha) {
  const seed = await gh("PUT", `${API}/contents/README.md`, JSON.stringify({
    ...
    content: Buffer.from("# DSH Arena\n\nDSH Arena - local experiment and evaluation workbench for DeepSeek Harness.\n").toString("base64")
  }));
